...
mod e2e_tests {
    ...
    #[allow(dead_code)]
    type Client = ink_e2e::Client<PolkadotConfig, ink::env::DefaultEnvironment>;
    ...
    async fn deployment_works(mut client: Client) -> E2EResult<()> {
        ...
        let contract = client
            .instantiate("assets_precompile_node", &alice(), &mut constructor)
    ...
    async fn transfer_works(mut client: Client) -> E2EResult<()> {
        ...
        let contract_balance = client.balance_of(&asset_id, &contract.account_id).await;
        let bob_balance = client.balance_of(&asset_id, &bob).await;
        assert_eq!(contract_balance, 99_000u128);
        assert_eq!(bob_balance, 1_000u128);

        // Attempt to transfer more than the contract's balance, should fail
        let result = client
            .call(
                &alice,
    ...
    async fn approve_works(mut client: Client) -> E2EResult<()> {
        ...
        let contract = client
            ...
            // Contract needs native balance for approval deposit (100_000_000).
            .value(1_000_000_000)
            .submit()
            .await?;
    ...
    async fn allowance_works(mut client: Client) -> E2EResult<()> {
        ...
        client.approve(&asset_id, &alice, &bob, 300u128).await?;

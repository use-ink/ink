...
#[ink::contract]
mod multi_contract_ref {
    use accumulator::AccumulatorRef;
    use adder::AdderRef;
    use subber::SubberRef;

    /// Specifies the state of the `multi_contract_ref` contract.
    ///
    /// In `Adder` state the `multi_contract_ref` contract will call the `Adder` contract
    ...
    /// # Note
    ///
    /// In order to instantiate the `multi_contract_ref` smart contract we first
    ...
    /// `multi_contract_ref` smart contract.
    ///
    ...
    impl MultiContractRef {
        /// Instantiate a `multi_contract_ref` contract with the given sub-contract codes.
        #[ink(constructor)]
        pub fn new(
            init_value: i32,
            version: u32,
        ...
        /// Switches the `multi_contract_ref` contract.
        #[ink(message)]
    ...
    mod e2e_tests {
        use super::MultiContractRefRef;
        use ink_e2e::build_message;

        type E2EResult<T> = std::result::Result<T, Box<dyn std::error::Error>>;

        #[ink_e2e::test]
        async fn e2e_multi_contract_ref(
            ...
            let constructor = MultiContractRefRef::new(
                1234, // initial value
                1337, // salt
                accumulator_hash,
                adder_hash,
            ...
            let multi_contract_ref_acc_id = client
                .instantiate(
                    "multi_contract_ref",
                    &ink_e2e::alice(),
                    constructor,
            ...
            // when
            let get =
                build_message::<MultiContractRefRef>(multi_contract_ref_acc_id.clone())

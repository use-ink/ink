...
//!
//! An AMM is a decentralized marketplace to swap tokens. Usually swap fees are significantly cheaper than centralized exchanges,
//! and this system allows anyone to participate as either the Swapper (a user interested in swapping one token for another)
//! or as a Liquidity Provide (LP), //! who is financially incentivised to deposit equal amounts of token1 and token2. Each time
//! a token is swapped a small percentage is given to all the LPs, and each share is proportional to the amount of liquidity
//! that a LP provides. Pretty simple, right? You can learn more about AMMs [here](https://www.coindesk.com/learn/2021/08/20/what-is-an-automated-market-maker/).
//!
//! This code example was inspired by [this blog](https://learn.figment.io/tutorials/build-polkadot-amm-using-ink).
//!
//! ## Warning
//!
...
//! ## Tokens and AMMs
//!
//! If you are familiar with Solidity you may have worked with AMM contracts that integrate multiple ERC20 contracts for
//! token pairs. This is a composable contract model. In this code example, however, we demonstrate a more simple version where all tokens
//! are maintained by this contract (one contract instead of many composed contracts).
//!
//! ## The Faucet
//!
//! There is a `faucet()` function built in to this contract to fund your wallet balance. You can use this to //! test dive the features
//! in a staging environment, and in tests.
//!
//! ## Error Handling
//!
...
/// This is a multiplier used to calculate the total LP Tokens a LP receives for providing liquidity to the pool
...
mod amm {
    ...
    #[ink(impl)]
    impl Amm {
        /// faucet is only used in a test scenario. Use this to add balances to your wallet for token1 and token2
        #[ink(message)]
        pub fn faucet(&mut self, amount_token1: Token1, amount_token2: Token2) {
            ...
            self.token1_balances
                .insert(&caller, &(token1 + amount_token1));
            self.token2_balances
                .insert(&caller, &(token2 + amount_token2));
        }
        ...
        /// Returns the total pool amount for token1, token2, and lp_tokens, and the pool swap fee
        #[ink(message)]
        ...
        /// To become a LP and start earning on swap rewards you must put liquidity into the pool using this function.
        /// NOTE: The first time you call this you must make sure that the two token amounts are of equal value.
        /// The first deposit is crucial to get right because it sets up a mathematical relationship between the two
        /// tokens assuming that they are of equal value.
        #[ink(message)]
        pub fn provide_liquidity(
            &mut self,
            token1_amount: Token1,
            token2_amount: Token2,
        ) -> Result<LPTokens, Error> {
            self.has_balance(&self.token1_balances, token1_amount)?;
            self.has_balance(&self.token2_balances, token2_amount)?;

            // If the total lp token amount is 0 it means that this is the first time someone is providing liquidity,
            // and the total lp tokens (shares) will be initialized at a constant amount.
        ...
        /// Returns the amount of token1 a caller can receive in exchange for the given amount of token2
        #[ink(message)]
        ...
        /// Returns the amount of token2 a caller can receive in exchange for the given amount of token1
        /// This does not account for the swap fee.
        /// Returns an error if the pool does not have any liquidity.
        #[ink(message)]
        ...
        /// Returns the amount of token1 and token2 a LP will receive in exchange for LP tokens
        /// This result should increase over time based on swap activity assuming that the pool fee is greater than 0
        #[ink(message)]
        pub fn get_withdraw_estimate(
            &self,
            lp_tokens: LPTokens,
        ) -> Result<(Token1, Token2), Error> {
            self.has_active_pool()?;
        ...
        /// This is used for LP to redeem the liquidity plus interest eared in exchange for LP tokens
        #[ink(message)]
        pub fn withdraw(&mut self, claim: LPTokens) -> Result<(Token1, Token2), Error> {
            let caller = self.env().caller();
            self.has_balance(&self.lp_token_balances, claim)?;

            let (amount_token1, amount_token2) = self.get_withdraw_estimate(claim)?;
            ...
            self.token1_balances
                .insert(&caller, &(amount_token1 + t1_balance));

            let t2_balance = self.token2_balances.get(&caller).unwrap_or(0);
            self.token2_balances
                .insert(&caller, &(amount_token2 + t2_balance));

            Ok((amount_token1, amount_token2))
        }

        /// Get the exchange amount for token2 given the amount in token1. This accounts for the pool fee.
        /// Returns an error if the pool liquidity is zero
        #[ink(message)]
        ...
        /// Get the exchange amount for token1 given the amount in token2. This accounts for the pool fee.
        /// Returns an error if the pool liquidity is zero
        #[ink(message)]
        ...
        /// Swap token1 in exchange for token2. You can pass in the min_token2_required to guard
        /// yourself from losing too much due to "slippage".
        #[ink(message)]
        ...
        /// Swap token2 in exchange for token1. You can pass in the max_token1_required to guard
        /// yourself from losing too much due to "slippage".
        #[ink(message)]
        ...
        /// Note how we init this struct using a function. Simply calling
        /// ink_lang::utils::initialize_contract(1000) will not create the correct storage keys
        /// "under the hood" for the database. The function format is required to initialize
        ...
        fn has_active_pool(&self) -> Result<(), Error> {

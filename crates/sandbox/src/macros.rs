...
///
/// This macro verifies that the last emitted contract event from the runtime backend
...
///
/// The new macro will automatically implement `crate::Runtime`.
#[macro_export]
macro_rules! create_runtime {
    ...
    ($backend:ident, $runtime:ident, $debug: ty, { $( $pallet_name:tt : $pallet:ident ),* $(,)? }) => {
...
mod construct_runtime {
    ...
    // Implement `crate::Runtime` trait
    impl $crate::Runtime for $backend {
...
pub use construct_runtime::{
    $backend, $runtime, Assets, AssetIdForTrustBackedAssets, Balances, Revive, PalletInfo,

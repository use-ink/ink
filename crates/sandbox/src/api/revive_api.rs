...
where
    ...
    <<T as Runtime>::Runtime as frame_system::Config>::Nonce: Into<u32>,
    // todo
    <<T as Runtime>::Runtime as frame_system::Config>::Hash:
...
mod tests {
    ...
    /// This function funds the account with the existential deposit
    /// (i.e. minimum balance).
    fn warm_up<T>(backend: &mut T)
    where
        <T as Runtime>::Runtime: pallet_revive::Config + pallet_balances::Config,
        T: BalanceAPI<T> + Runtime,
    {
        let acc = pallet_revive::Pallet::<<T as Runtime>::Runtime>::account_id();
        let ed = pallet_balances::Pallet::<<T as Runtime>::Runtime>::minimum_balance();
        backend.mint_into(&acc, ed).unwrap_or_else(|_| {
    ...
    fn can_upload_code() {
        ...
        warm_up::<DefaultRuntime>(&mut backend);
        ...
        let result = backend.upload_contract(contract_binary, origin, 100000000000000);
    ...
    fn can_deploy_contract() {
        ...
        backend.map_account(&default_actor).expect("cannot map");
        let origin = DefaultRuntime::convert_account_to_origin(default_actor);
        let result = backend.deploy_contract(
    ...
    fn can_call_contract() {
        ...
        let default_actor = DefaultRuntime::default_actor();
        backend.map_account(&default_actor).expect("unable to map");
        let origin = DefaultRuntime::convert_account_to_origin(default_actor);
        let result = backend.deploy_contract(
            contract_binary,
            0,
        ...
        let result = backend.call_contract(
            contract_address,
            0,
            vec![],
            origin.clone(),

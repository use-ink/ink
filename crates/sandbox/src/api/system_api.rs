...
/// System API for the runtime backend.
pub trait SystemAPI {
...
mod tests {
    ...
    fn make_transfer(
        backend: &mut DefaultRuntime,
        dest: AccountId32,
        value: u128,
    ) -> DispatchResultWithInfo<
        <RuntimeCall<<DefaultRuntime as Runtime>::Runtime> as Dispatchable>::PostInfo,
    > {
        assert_ne!(
    ...
    fn dry_run_works() {
        ...
        let initial_balance = backend.free_balance(&actor);

        backend.dry_run(|backend| {
            crate::api::balance_api::BalanceAPI::mint_into(backend, &actor, 100).unwrap();
            assert_eq!(backend.free_balance(&actor), initial_balance + 100);
        });

        assert_eq!(backend.free_balance(&actor), initial_balance);
    }
    ...
    fn runtime_call_works() {
        ...
        let initial_balance = backend.free_balance(&RECIPIENT);

        let result = make_transfer(&mut backend, RECIPIENT, 100);
        assert!(result.is_ok());

        let expected_balance = initial_balance + 100;
        assert_eq!(backend.free_balance(&RECIPIENT), expected_balance);
    }
    ...
    fn current_events() {
        ...
        let events_before = backend.events();
        assert!(events_before.is_empty());

        make_transfer(&mut backend, RECIPIENT, 1).expect("Failed to make transfer");

        let events_after = backend.events();
    ...
    fn resetting_events() {
        ...
        make_transfer(&mut backend, RECIPIENT.clone(), 1)
    ...
    fn snapshot_works() {
        ...
        // Check state before
        let block_before = backend.block_number();
        let snapshot_before = backend.take_snapshot();

        // Advance some blocks to have some state change
        let _ = backend.build_blocks(5);
        let block_after = backend.block_number();

        // Check block number and state after
        assert_eq!(block_before + 5, block_after);

        // Restore state
        backend.restore_snapshot(snapshot_before);

        // Check state after restore
        assert_eq!(block_before, backend.block_number());
    }
}

...
/// Assets API for the runtime backend.
///
...
mod tests {
    ...
    #[test]
    fn create_works() {
        let mut backend = DefaultRuntime::default();
        let admin = DefaultRuntime::default_actor();
        let asset_id = 1u32;
        let min_balance = 1u128;

        let result = backend.create(&asset_id, &admin, min_balance);

        assert!(result.is_ok());
        assert!(backend.asset_exists(&asset_id));
    }
    ...
    fn set_metadata_works() {
        ...
        backend.create(&asset_id, &admin, 1u128).unwrap();

        let name = b"Test Token".to_vec();
        let symbol = b"TEST".to_vec();
        let decimals = 18u8;

        let result = backend.set_metadata(&asset_id, &admin, name, symbol, decimals);
    ...
    fn approve_works() {
        ...
        backend.create(&asset_id, &admin, 1u128).unwrap();
        backend.mint_into(&asset_id, &admin, 1000u128).unwrap();

        let allowance_before = backend.allowance(&asset_id, &admin, &spender);
        assert_eq!(allowance_before, 0);

        let result = backend.approve(&asset_id, &admin, &spender, 500u128);

        assert!(result.is_ok());

        let allowance_after = backend.allowance(&asset_id, &admin, &spender);
    ...
    fn mint_into_works() {
        ...
        let balance_before = backend.balance_of(&asset_id, &admin);
        assert_eq!(balance_before, 0);

        backend.mint_into(&asset_id, &admin, 100u128).unwrap();

        let balance_after = backend.balance_of(&asset_id, &admin);
    ...
    fn transfer_works() {
        ...
        let admin_balance_before = backend.balance_of(&asset_id, &admin);
        let recipient_balance_before = backend.balance_of(&asset_id, &recipient);

        assert_eq!(admin_balance_before, 1000);
        assert_eq!(recipient_balance_before, 0);

        let result = backend.transfer(&asset_id, &admin, &recipient, 300u128);

        assert!(result.is_ok());

        let admin_balance_after = backend.balance_of(&asset_id, &admin);
        let recipient_balance_after = backend.balance_of(&asset_id, &recipient);
    ...
    fn balance_of_works() {
        ...
        let balance = backend.balance_of(&asset_id, &admin);
        assert_eq!(balance, 0);

        backend.mint_into(&asset_id, &admin, 500u128).unwrap();
    ...
    fn total_supply_works() {
        ...
        let supply_before = backend.total_supply(&asset_id);
        assert_eq!(supply_before, 0);

        backend.mint_into(&asset_id, &admin, 1000u128).unwrap();

        let supply_after = backend.total_supply(&asset_id);
    ...
    fn allowance_works() {
        ...
        let allowance = backend.allowance(&asset_id, &admin, &spender);

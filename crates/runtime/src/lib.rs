...
/// Alias for the runtime of a runtime environment.
pub type RuntimeOf<S> = <S as RuntimeEnv>::Runtime;

/// Alias for the runtime event of a runtime environment.
pub type RuntimeEventOf<S> = <RuntimeOf<S> as frame_system::Config>::RuntimeEvent;

/// RuntimeEnv defines the API of a runtime environment.
pub trait RuntimeEnv {
    /// The runtime associated with the runtime environment.
    ...
    /// Default actor for the runtime environment.
...
/// Convert a `pallet_revive::CallTrace` (runtime) into an `ink_revive_types::CallTrace`
/// (API).

...
fn storable_struct_derive(s: &synstructure::Structure) -> TokenStream2 {
    ...
    let decode_body = variant.construct(|field, _index| {
        ...
        let span = ty.span();
        quote_spanned!(span =>
            <#ty as ::ink::storagee::traits::Storable>::decode(__input)?
        )
    });
    let encode_body = variant.each(|binding| {
        let span = binding.ast().ty.span();
        quote_spanned!(span =>
            ::ink::storagee::traits::Storable::encode(#binding, __dest);
        )
    });

    s.gen_impl(quote! {
         gen impl ::ink::storagee::traits::Storable for @Self {
            #[inline(always)]
...
fn storable_enum_derive(s: &synstructure::Structure) -> TokenStream2 {
    ...
    let encode_body = s.variants().iter().enumerate().map(|(index, variant)| {
        ...
        let fields = variant.bindings().iter().map(|field| {
            ...
            quote_spanned!(span =>
                ::ink::storagee::traits::Storable::encode(#field, __dest);
            )
        });
        quote! {
             #pat => {
                 { <::core::primitive::u8 as ::ink::storagee::traits::Storable>::encode(&#index, __dest); }
                 #(
                     { #fields }
                 )*
             }
         ...
         gen impl ::ink::storagee::traits::Storable for @Self {
            ...
            fn decode<__ink_I: ::scale::Input>(__input: &mut __ink_I) -> ::core::result::Result<Self, ::scale::Error> {
                ::core::result::Result::Ok(
                    match <::core::primitive::u8 as ::ink::storagee::traits::Storable>::decode(__input)? {
                        #decode_body

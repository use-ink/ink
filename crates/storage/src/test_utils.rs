...
/// Creates two tests:
/// (1) Tests if an object which is `push_spread`-ed to storage results in
/// exactly     the same object when it is `pull_spread`-ed again. Subsequently
/// the object     undergoes the same test for `push_packed` and `pull_packed`.
...
///     - Push `instance2` to storage, pull it out and assert that what is
///       pulled out is what was pushed.
///     - Do some mutations on the `pulled` object. Here the two tests behave
///       differently:
///
///         * `fuzz_ $id _mutate_some` Mutates some entries of the data
///           structure based on the content of `instance2`.
///
///         * `fuzz_ $id _mutate_all` Mutates the entire data structure, so that
///           it has the same content as `instance2`.
///
///     - Push the mutated `pulled` object into storage again, pull it out as
///       `pulled2` and assert that both objects are equal.
///     - Clear the object from storage and assert that storage was cleared up
///       properly, without any leftovers.
#[macro_export]

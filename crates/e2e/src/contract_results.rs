...
pub struct CallDryRunResult<E: Environment, V, Abi> {
    ...
    pub _marker: RetTypeMarker<V, Abi>,
}

/// Phantom type for return type and its ABI encoding.
pub struct RetTypeMarker<V, Abi> {
    _ty_marker: PhantomData<V>,
    _abi_marker: PhantomData<Abi>,
}

// This is a manual implementation because the derived implementation requires an
// unnecessary `V: Default` bound.
impl<V, Abi> Default for RetTypeMarker<V, Abi> {
    fn default() -> Self {
        Self {
            _ty_marker: Default::default(),
            _abi_marker: Default::default(),
        }
    }
}
...
impl<E: Environment, V: SolDecode> CallDryRunResult<E, V, SolEncoding> {
    ...
    pub fn return_value(&self) -> V {
        ...
        SolDecode::decode(&mut data.as_ref()).unwrap_or_else(|err| {
            panic!(

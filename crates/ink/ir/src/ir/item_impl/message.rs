...
mod tests {
    ...
    fn allow_reentrancy_works() {
        ...
        for (expect_allow_reentrancy, item_method) in test_inputs {
            let allow_reentrancy = <ir::Message as TryFrom<_>>::try_from(item_method)
                .unwrap()
                .allow_reentrancy();
            assert_eq!(allow_reentrancy, expect_allow_reentrancy);
        }
    }

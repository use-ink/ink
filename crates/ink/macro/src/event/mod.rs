...
pub use self::metadata::event_metadata_derive;
...
fn event_derive_struct(mut s: synstructure::Structure) -> syn::Result<TokenStream2> {
    ...
    Ok(generate_abi_impls!(@type |abi| {
        ...
        let event_signature_topic = if anonymous {
            ...
            let value = match abi {
                Abi::Ink => quote!(Self::SIGNATURE_TOPIC.as_ref()),
                Abi::Sol => {
                    quote!(Self::SIGNATURE_TOPIC
                        .as_ref()
        ...
        let signature_topic = if !anonymous {
            ...
            match abi {
                ...
                Abi::Sol => {
                    let calculated_signature_topic =
                        signature_topic_sol(variant.ast().fields, event_ident);
                    quote_spanned!(span=> ::core::option::Option::Some(#calculated_signature_topic))
                }
            }
        } else {
...
fn signature_topic_sol(fields: &syn::Fields, event_ident: &syn::Ident) -> TokenStream2 {
    ...
    let sig_str = quote! {
        ::ink::codegen::utils::const_format!(#sig_fmt_str, #name #(,#param_tys)*)
    };
    quote!(::ink::codegen::sol::keccak_256(#sig_str))
}

...
use crate::codegen::TraitCallBuilder;
use ink_env::{
    call::{
        state::Salt,
...
) -> CreateParams<E, ContractRef, Args, Vec<u8>, R>
where
    E: Environment,
{
    builder
...
pub fn constructor_exec_input<E, ContractRef, Args: Encode, R>(
    param_builder: &CreateParams<E, ContractRef, Args, Vec<u8>, R>,
) -> Vec<u8>
where
    E: Environment,
{
    // set all the other properties to default values, we only require the `exec_input`.
    param_builder.exec_input().encode()
}

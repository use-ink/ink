    ...
    fn generate_trait_message_property_guards(&self) -> TokenStream2 {
        ...
        let trait_message_guards = self
            ...
            .map(|(trait_path, message)| {
                ...
                let message_guard_allow_reentrancy = message.allow_reentrancy().then(|| {
                    quote_spanned!(message_span=>
                        const _: ::ink::codegen::TraitMessageAllowReentrancy<{
                            <<::ink::reflect::TraitDefinitionRegistry<<#storage_ident as ::ink::reflect::ContractEnv>::Env>
                                as #trait_path>::__ink_TraitInfo
                                as ::ink::reflect::TraitMessageInfo<#message_local_id>>::ALLOW_REENTRANCY
                        }> = ::ink::codegen::TraitMessageAllowReentrancy::<true>;
                    )
                });
                ...
                quote_spanned!(message_span=>
                    ...
                    #message_guard_allow_reentrancy

...
//!
//! These are simple mirrored types from the default substrate FRAME
//! configuration. Their interfaces and functionality might not be complete.
//!
//! Users are required to provide their own type definitions and `Environment`
//! implementations in order to write ink! contracts for other chain
//! configurations.
//!
//! # Note
//!
//! When authoring a contract, the concrete `Environment` are available via
//! aliases generated by the `lang` macro. Therefore all functionality of the
//! concrete types is accessible in the contract, not constrained by the
//! required trait bounds.
//!
//! Outside the contract and its tests (e.g. in the off-chain environment),
//! where there is no knowledge of the concrete types, the functionality is
//! restricted to the trait bounds on the `Environment` trait types.
...
pub trait Environment {
    ...
    ///
    /// The value must match the maximum number of supported event topics of the
    /// used runtime.
    ...
    /// procedural macro. For more information about usage and definition
    /// click [this][chain_extension] link.
    ///
...
/// Information needed for rent calculations that can be requested by a
/// contract.
#[derive(scale::Decode)]
#[cfg_attr(test, derive(Debug, PartialEq))]
pub struct RentParams<T: Environment> {
    /// The total balance of the contract. Includes the balance transferred from
    /// the caller.
    pub total_balance: T::Balance,

    /// The free balance of the contract, i.e. the portion of the contract's
    /// balance that is not reserved. Includes the balance transferred from
    /// the caller.
    pub free_balance: T::Balance,

    /// Subsistence threshold is the extension of the minimum balance (aka
    /// existential deposit) by the tombstone deposit, required for leaving
    /// a tombstone.
    ///
    /// Rent or any contract initiated balance transfer mechanism cannot make
    /// the balance lower than the subsistence threshold in order to
    /// guarantee that a tombstone is created.
    ///
    /// The only way to completely kill a contract without a tombstone is
    /// calling `seal_terminate`.
    pub subsistence_threshold: T::Balance,

    /// The balance every contract needs to deposit to stay alive indefinitely.
    ///
    /// This is different from the tombstone deposit because this only needs to
    /// be deposited while the contract is alive. Costs for additional
    /// storage are added to this base cost.
    ///
    /// This is a simple way to ensure that contracts with empty storage
    /// eventually get deleted by making them pay rent. This creates an
    /// incentive to remove them early in order to save rent.
    pub deposit_per_contract: T::Balance,

    /// The balance a contract needs to deposit per storage byte to stay alive
    /// indefinitely.
    ///
    /// Let's suppose the deposit is 1,000 BU (balance units)/byte and the rent
    /// is 1 BU/byte/day, then a contract with 1,000,000 BU that uses 1,000
    /// bytes of storage would pay no rent. But if the balance reduced to
    /// 500,000 BU and the storage stayed the same at 1,000, then it would
    /// pay 500 BU/day.
    ...
    /// The contract's rent allowance, the rent mechanism cannot consume more
    /// than this.
    pub rent_allowance: T::Balance,

    /// The fraction of the deposit costs that should be used as rent per block.
    ///
    /// When a contract does not have enough balance deposited to stay alive
    /// indefinitely it needs to pay per block for the storage it consumes
    /// that is not covered by the deposit. This determines how high this
    /// rent payment is per block as a fraction of the deposit costs.
...
pub struct RentStatus<T: Environment> {
    /// Required deposit assuming that this contract is the only user of its
    /// code.
    ...
    /// Required deposit assuming the specified `refcount` (`None` if `0` is
    /// supplied).
    pub custom_refcount_deposit: Option<T::Balance>,

    /// Rent that is paid assuming that the contract is the only user of its
    /// code.
    ...
    /// Rent that is paid assuming the specified refcount (`None` if `0` is
    /// supplied).

...
/// The flags to indicate further information about the end of a contract
/// execution.
#[derive(Default)]
...
pub trait EnvBackend {
    ...
    /// # Note
    ///
    /// - The input is the 4-bytes selector followed by the arguments of the
    ///   called function in their SCALE encoded representation.
    ...
    /// to their exported constructors and messages that implement
    /// `scale::Decode` according to the constructors or messages selectors
    /// and their arguments. These `enum` dispatch types are then given to
    /// this procedure as the `T`.
    ///
    ...
    ///
    /// The message is appended to the debug buffer which is then supplied to
    /// the calling RPC client. This buffer is also printed as a debug
    /// message to the node console if the `debug` log level is enabled for
    /// the `runtime::contracts` target.
    ///
    /// If debug message recording is disabled in the contracts pallet, which is
    /// always the case when the code is executing on-chain, then this will
    /// have no effect.
    ...
    /// Conducts the crypto hash of the given encoded input and stores the
    /// result in `output`.
...
pub trait TypedEnvBackend: EnvBackend {
    ...
    /// [`transferred_balance`][`crate::transferred_balance`]
    ...
    ///
    /// For more details visit:
    /// [`tombstone_deposit`][`crate::tombstone_deposit`]
    ...
    ///
    /// For more details visit:
    /// [`set_rent_allowance`][`crate::set_rent_allowance`]
    ...
    ///
    /// For more details visit:
    /// [`instantiate_contract`][`crate::instantiate_contract`]
    ...
    ///
    /// For more details visit:
    /// [`terminate_contract`][`crate::terminate_contract`]

...
/// A Trace
#[derive(
    TypeInfo, From, Encode, Decode, Serialize, Deserialize, Clone, Debug, Eq, PartialEq,
)]
#[serde(untagged)]
pub enum Trace {
    /// A call trace.
...
/// A prestate Trace
#[derive(
    TypeInfo, Encode, Decode, Serialize, Deserialize, Clone, Debug, Eq, PartialEq,
)]
#[serde(untagged)]
pub enum PrestateTrace {
    ...
    DiffMode {
        ...
        ///  The accounts in the `pre` field will contain all of their basic fields, even
        /// if those fields have not been modified. For `storage` however, only
        /// non-empty slots that have been modified will be included
        pre: BTreeMap<H160, PrestateTraceInfo>,
        /// The state after the call.
        /// It only contains the specific fields that were actually modified during the
        /// transaction
...
impl PrestateTrace {
    ...
    pub fn state_mut(
        ...
        &mut BTreeMap<H160, PrestateTraceInfo>,
        Option<&mut BTreeMap<H160, PrestateTraceInfo>>,
    ) {
        match self {

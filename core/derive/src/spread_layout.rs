    ...
    let variant_footprints = s
        ...
        .map(|variant| {
            variant
                ...
                .iter()
                .map(|field| &field.ty)
                .map(|ty| quote! { <#ty as ::ink_core::storage::traits::SpreadLayout>::FOOTPRINT })
...
fn requires_deep_clean_up(s: &synstructure::Structure) -> TokenStream2 {
    s.variants()
        ...
        .map(|variant| {
            ...
            .iter()
            .map(|field| &field.ty)
            .map(|ty| quote! { <#ty as ::ink_core::storage::traits::SpreadLayout>::REQUIRES_DEEP_CLEAN_UP })
...
fn spread_layout_struct_derive(s: &synstructure::Structure) -> TokenStream2 {
    ...
    let pull_body = variant.construct(|field, _index| {
        let ty = &field.ty;
        quote! {
            <#ty as ::ink_core::storage::traits::SpreadLayout>::pull_spread(__key_ptr)
        }
    });
    let push_body = variant.each(|binding| {
        quote! {
            ::ink_core::storage::traits::SpreadLayout::push_spread(#binding, __key_ptr);
        }
    });
    let clear_body = s.each(|field| {
        quote! {
            ::ink_core::storage::traits::SpreadLayout::clear_spread(#field, __key_ptr);
        }
    });
    s.gen_impl(quote! {
        gen impl ::ink_core::storage::traits::SpreadLayout for @Self {
...
fn spread_layout_enum_derive(s: &synstructure::Structure) -> TokenStream2 {
    ...
    let push_body = s.variants().iter().enumerate().map(|(index, variant)| {
        ...
        let fields = variant.bindings().iter().map(|field| {
            quote! {
                ::ink_core::storage::traits::SpreadLayout::push_spread(#field, __key_ptr);
            }
        });
        quote! {
            #pat => {
                { <u8 as ::ink_core::storage::traits::SpreadLayout>::push_spread(&#index, __key_ptr); }
                #(
                    { #fields }
                )*
            }

    ...
    V: scale::Codec  + core::fmt::Debug,
{
    ...
    pub fn get(&self, key: &K) -> Option<&V> {
        match search::search_tree(&self, self.header.root, &key) {
            SearchResult::Found(handle) => {
                let idx = handle.idx();
                let node = self.get_node_by_handle(&handle.into())
    ...
    pub fn print_tree(&self) {
        for i in 0..self.header.node_count {
            let nd = NodeHandle(i);
            let node = self.get_node_by_handle(&nd).expect("printing all nodes");
            eprintln!(
                "{:?}\nkeys: {:?}\nvals: {:?}\nedges: {:?}\nparent: {:?}\nparent idx: {:?}\n",
    ...
    // ToDo duplicate code
    pub(crate) fn descend(&self, handle: KVHandle) -> Option<NodeHandle> {
        let node = self.get_node_by_handle(&handle.into())
    ...
    // ToDo duplicate code
    pub(crate) fn get_node_by_handle(&self, handle: &NodeHandle) -> Option<&Node<K, V>> {
    ...
    V: Encode + Decode  + core::fmt::Debug,
{
    ...
    /// `edge.descend().ascend().unwrap()` and `node.ascend().unwrap().descend()` should
    /// both, upon success, do nothing.
    fn descend(&self, handle: &KVHandle) -> Option<NodeHandle> {
    ...
    fn split_leaf(&mut self, idx: usize) -> (K, V, NodeHandle) {
        let mut node = self.get_node_mut(&self.handle.into())
    ...
    fn get_node(&self, handle: &NodeHandle) -> Option<&Node<K, V>> {
    ...
    V: Encode + Decode ,
{
    ...
    fn node(&self) -> Option<&Node<K, V>> {
        let entry = self.entries.get(self.handle.node)
    ...
    fn node_mut(&mut self) -> Option<&mut Node<K, V>> {
        let entry = self.entries.get_mut(self.handle.node)
            .expect("node to get kv pairs from must exist");
        match entry {
            InternalEntry::Occupied(occupied) => Some(occupied),
            InternalEntry::Vacant(_) => None,
    ...
    fn kv(&self) -> (&K, &V) {
        let node = self.node().expect("node on OccupiedEntry must exist");
    ...
    fn kv_mut(&mut self) -> (&mut K, &mut V) {
        let idx = self.handle.idx();
        let node = self.node_mut().expect("node on OccupiedEntry must exist");
    ...
    fn into_kv_mut(self) -> (&'a mut K, &'a mut V) {
        ...
        let node = match entry {
            InternalEntry::Occupied(occupied) => Some(occupied),
            InternalEntry::Vacant(_) => None,
        }.expect("internal entry to get kv pairs from must exist");
        let k = node.keys[idx].as_mut().expect("key must exist");
        let v = node.vals[idx].as_mut().expect("value must exist");
        (k, v)
    }
